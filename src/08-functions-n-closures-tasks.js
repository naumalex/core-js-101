 ...
 *   getPolynom()      => null
 */
function getPolynom(...koeffs) {
  return (x) => {
    let sum = 0;
    let power = koeffs.length - 1;
    for (let i = 0; i < koeffs.length; i += 1) {
      sum += koeffs[i] * (x ** power);
      power -= 1;
    }
    return sum;
  };
  // koeffs.reduce((element) => element * x ** koeffs);
}
...
function retry(func, attempts) {
  return function retried(...args) {
    const exceptions = [];
    for (let attempted = 0; attempted < attempts; attempted += 1) {
      try {
        return func(...args);
      } catch (ex) {
        exceptions.push(ex);
...
function logger(func, logFunc) {
  return function logged(...args) {
    ...
    logFunc(`${func.name}(${params.toString()}) starts`);
    const a = func(...args);
    logFunc(`${func.name}(${params.toString()}) ends`);
    return a;
  };
}
